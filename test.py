...
from IMDLBenCo.model_zoo.FMAE.FMAE import FMAE
...
def main(args, model_args):
    ...
    for dataset_name, dataset_path in test_dataset_json.items():
        ...
        print(chkpt_list)
        chkpt_pair = [(int(chkpt.split('-')[1].split('.')[0]) , chkpt) for chkpt in chkpt_list if chkpt.endswith(".pth") and 'last' not in chkpt]
        chkpt_pair.sort(key=lambda x: x[0])
        print( "sorted checkpoint pairs in the ckpt dir: ",chkpt_pair)
        for epoch_name, chkpt_dir in chkpt_pair:
