...
class ImageAUC(AbstractEvaluator):
    ...
    def epoch_update(self, **kwargs):
        if kwargs['distributed']:
            ...
            AUC = roc_auc_score(self.label.detach().cpu().numpy(), self.predict_label.detach().cpu().numpy()).item()
        return AUC
...
class PixelAUC(AbstractEvaluator):
    ...
    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        ...
        elif self.mode == "skmetric":
            for idx in range(predict.shape[0]):
                try:
                    auc_item = roc_auc_score(mask[idx].squeeze(0).flatten().detach().cpu().numpy(), predict[idx].squeeze(0).flatten().detach().cpu().numpy()).item()
                except:
                    print('only one class y_true')
                    auc_item = 0
                AUC_list.append(auc_item)
        else:
...
def test_origin_image_auc():
    ...
    gpu_f1_score = evaluator.epoch_update(distributed=True)
    if(dist.get_rank() == 0):
        print(f"AUC Score: {gpu_f1_score}")
        print(f"{roc_auc_score(all_labels[:-50], (all_predicts[:-50]))}")
