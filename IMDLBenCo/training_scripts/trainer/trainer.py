...
def train_one_epoch(model: torch.nn.Module,
    ...
    print("args.if_not_amp:", args.if_not_amp)
    ...
    for data_iter_step, data_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # ============================LTSFT===============================
        loss_scaler(predict_loss, optimizer, parameters=model.parameters(), 
                    update_grad=False, backward_loss=True)
        with torch.no_grad():
            for name, params in model.named_parameters():
                if name in grad_mask.keys():
                    mask = nn.Parameter(grad_mask[name], requires_grad=False)
                    params.grad.mul_(mask)
        loss_scaler(predict_loss,optimizer, parameters=model.parameters(),
                    update_grad=(data_iter_step + 1) % accum_iter == 0, backward_loss=False)
