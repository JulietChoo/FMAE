...
from IMDLBenCo.datasets import ManiDataset, ManiAuDataset, UnBalancedAuDataset, UnBalancedDataset
from IMDLBenCo.evaluation import PixelF1, PixelAUC, ImageAccuracy, ImageAUC, ImageEER # TODO You can select evaluator you like here

from IMDLBenCo.training_scripts.tester import test_one_epoch
from IMDLBenCo.training_scripts.trainer import train_one_epoch

import IMDLBenCo.utils.lr_decay as lrd
...
def get_args_parser():
    ...
    parser.add_argument('--if_detect', action='store_true', 
                        help='detect')
    ...
    parser.add_argument('--gamma', type=float, default=0,
                        help='weight loss')
    parser.add_argument('--sample_number', default=12614, type=int,
                        help='number of distributed processes')
    parser.add_argument('--all_number', default=15137, type=int,
                        help='number of distributed processes')
    parser.add_argument('--layer_decay', type=float, default=0.75,
                        help='layer-wise lr decay from ELECTRA/BEiT')
    parser.add_argument('--det_resume_ckpt', default = None, type=str, help='path to det pretrain model')
    parser.add_argument('--gradmask_path', default=None, type=str)
    parser.add_argument('--gradmask_ratio', default=0.3, type=float) #grad_mask_ratio表示梯度冻结占比，0表示所有的参数正常优化，0.3表示30%参数冻结，70%参数正常优化
...
def main(args, model_args):
    ...
    if not args.if_detect:
        ...
        if os.path.isdir(args.data_path):
            ...
            print('Using UnBalancedDataset')
            dataset_train = UnBalancedDataset(
                sample_number = args.sample_number,
                all_number = args.all_number,
                is_padding=args.if_padding,
                is_resizing=args.if_resizing,
                output_size=(args.image_size, args.image_size),
                common_transforms=train_transform,
        ...
        if os.path.isdir(args.data_path):
            ...
            dataset_train = UnBalancedAuDataset(
                sample_number = args.sample_number,
                all_number = args.all_number,
                is_padding=args.if_padding,
                is_resizing=args.if_resizing,
    ...
    # model = FMAE(
    #     det_resume_ckpt = args.det_resume_ckpt
    # )
    ...
    if not args.if_detect:
        evaluator_list = [
            ...
            PixelAUC(threshold=0.5, mode="double")
        ]
    else:
        evaluator_list = [
            ImageAccuracy(threshold=0.5),
            ImageAUC(threshold=0.5),
            ImageEER(threshold=0.5)
        ]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer, grad_mask, device, epoch, loss_scaler,
            ...
            if not args.if_detect:  # localization
                if evaluate_metric_value > 0.41:
                    if evaluate_metric_value > best_evaluate_metric_value:
                        ...
                        best_epoch = epoch
                    print(f"Saving {evaluate_metric_for_ckpt} = {evaluate_metric_value}")
                    print(f"Best {evaluate_metric_for_ckpt} = {best_evaluate_metric_value}")
                    misc.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, epoch_name=epoch)
            else:   # detection
                if evaluate_metric_value > 0.735:
                    if evaluate_metric_value > best_evaluate_metric_value:
                        best_evaluate_metric_value = evaluate_metric_value
                        best_epoch = epoch
                    print(f"Saving epoch {evaluate_metric_for_ckpt} = {best_evaluate_metric_value}")
                    misc.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, epoch_name=epoch)
